...
public final class ChatFeatures {
	...
	public void onHotkey(int index) {
		...
		if (text.isEmpty()) {
			Mc.actionBar(Component.literal("Text-Hotkey " + (index + 1) + " ist leer"));
			return;
		}
		if (!hotkeys.tryAcquire(System.currentTimeMillis())) {
			Mc.actionBar(Component.literal("Text-Hotkey: zu schnell hintereinander"));
			return;
		}
		Mc.sendChat(text);
	}
	...
	public boolean onChatClick(double mouseX, double mouseY, boolean control) {
		...
		try {
			...
			Mc.actionBar(Component.literal("Zeile kopiert"));
			return true;
		} catch (RuntimeException e) {
