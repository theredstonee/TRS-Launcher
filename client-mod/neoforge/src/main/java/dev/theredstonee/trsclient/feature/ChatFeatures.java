...
public final class ChatFeatures {
	...
	public Component onMessage(Component message) {
		...
		if (modules.chat.isEnabled()) {
				...
				if (count > 1) {
					...
					try {
						...
						if (ChatLines.removeNewestIfSame(lastAdded)) {
							result = Component.literal("").append(message)
									.append(Component.literal(ChatStacker.suffix(count)).withStyle(ChatFormatting.GRAY));
						} else {
							stacker.reset();
			...
			if (modules.chatTimestamps.get()) {
				...
				result = Component.literal(stamp).withStyle(ChatFormatting.DARK_GRAY).append(result);
			}
		}
	...
	public void onHotkey(int index) {
		...
		if (text.isEmpty()) {
			Mc.actionBar(Component.literal(I18n.tr("chat.hotkeyEmpty", index + 1)));
			return;
		}
		if (!hotkeys.tryAcquire(System.currentTimeMillis())) {
			Mc.actionBar(Component.literal(I18n.tr("chat.hotkeyTooFast")));
			return;
		}
		Mc.sendChat(text);
	}
	...
	public boolean onChatClick(double mouseX, double mouseY, boolean control) {
		...
		try {
			...
			Mc.actionBar(Component.literal(I18n.tr("chat.lineCopied")));
			return true;
		} catch (RuntimeException e) {
