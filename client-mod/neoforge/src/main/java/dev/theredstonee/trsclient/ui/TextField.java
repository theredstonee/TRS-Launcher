 ...
 * Aufbau und Eingabe der Vanilla-Felder haben sich zwischen den Versionen mehrfach geändert,
 * diese Variante braucht nur {@code Gfx} und die Zeichen-/Tasten-Ereignisse des Bildschirms.
 */
public final class TextField {
	...
	public void draw(Gfx g, Font font, int x, int y, int w, int h, String placeholder) {
		...
		if (value.isEmpty() && !focused) {
			g.text(font, Gfx.clip(font, placeholder, w - 8), x + 4, textY, Brand.TEXT_DIM, false);
			return;
		}
		String shown = Gfx.clip(font, value, w - 8);
	...
	private void paste() {
		String clip = Mc.mc().keyboardHandler.getClipboard();
		if (clip == null) return;
		clip = clip.replace('\n', ' ').replace('\r', ' ');
