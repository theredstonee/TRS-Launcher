...
public final class KeystrokesHud extends HudElement {
	...
	public void draw(FontRenderer font, boolean preview) {
		...
		key(font, KEY + GAP, 0, KEY, KEY, label(font, 0, move[0]), move[0].getIsKeyPressed(), bg, color, module.shadow());
		int y = KEY + GAP;
		for (int i = 1; i < 4; i++) {
			key(font, (i - 1) * (KEY + GAP), y, KEY, KEY, label(font, i, move[i]), move[i].getIsKeyPressed(), bg, color, module.shadow());
		}
		y += KEY + GAP;
		...
		mouse(font, 0, y, mh, dev.theredstonee.trsclient.core.i18n.I18n.tr("hud.keystrokes.lmb"), mc.gameSettings.keyBindAttack.getIsKeyPressed(), bg, color, module.shadow(),
				showCps ? TrsClient.get().leftClicks().count(now) : -1);
		mouse(font, MOUSE_W + GAP, y, mh, dev.theredstonee.trsclient.core.i18n.I18n.tr("hud.keystrokes.rmb"), mc.gameSettings.keyBindUseItem.getIsKeyPressed(), bg, color, module.shadow(),
				showCps ? TrsClient.get().rightClicks().count(now) : -1);
		y += mh;

		if (modules.keystrokesShowSpace.get()) {
			y += GAP;
			boolean down = mc.gameSettings.keyBindJump.getIsKeyPressed();
