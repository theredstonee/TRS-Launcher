...
public final class PackScreen extends TrsScreen {
	...
	private void loadPacks() {
		entries.clear();
		fixedIds.clear();
		entries.addAll(Mc.availablePacks(fixedIds));
		enabledIds = Mc.selectedPackIds();
	...
	protected void draw(Gfx g, int mouseX, int mouseY, float partialTick) {
		...
		int sy = py + 30;
		int sw = pw - 20 - 96;
		g.fill(sx, sy, sx + sw, sy + 16, 0xFF000000);
		...
		if (query.isEmpty()) {
			...
			g.text(font, shown + (caret ? "_" : ""), sx + 4, sy + 4, Brand.TEXT, false);
		}
		...
		List<PackList.Entry> visible = PackList.visible(entries, enabledIds, filter, query);
	...
	protected boolean onKey(int key, int modifiers) {
		if (key == GLFW.GLFW_KEY_BACKSPACE && !query.isEmpty()) {
			query = hasControlDown() ? "" : query.substring(0, query.length() - 1);
			scroll = 0;
			return true;
		}
		return false;
	...
	protected boolean onChar(char c) {
		if (c < ' ' || c == 127 || query.length() >= MAX_QUERY) return false;
		query += c;
		scroll = 0;
		return true;
	}
