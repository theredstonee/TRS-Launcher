...
public final class TrsMenuScreen extends TrsScreen {
	...
	public TrsMenuScreen(Screen parent) {
		super("TRS Client");
		this.parent = parent;
		this.selected = TrsClient.get().visibleModules().get(0);
	}
	...
	protected void draw(Gfx g, int mouseX, int mouseY, float partialTick) {
		...
		int active = 0;
		for (Module m : TrsClient.get().visibleModules()) if (m.isEnabled()) active++;
		String status = active + "/" + TrsClient.get().visibleModules().size() + " aktiv";
	...
	private void drawCards(Gfx g, int mx, int my, int x, int y, int w, int h) {
		...
		cardsH = h;
		int cardW = (w - GAP) / 2;
		List<Module> all = TrsClient.get().visibleModules();
		...
		for (int i = 0; i < all.size(); i++) {
			...
			g.text(font, Gfx.trim(font, m.name(), cardW - 16), cx + 6, cy + 4, Brand.TEXT, false);
	...
	private void drawSettings(Gfx g, int mx, int my, int x, int y, int w, int h) {
		...
		for (String line : Gfx.wrap(font, selected.description(), iw)) {
