...
public final class WaypointEditScreen extends TrsScreen {
	...
	public WaypointEditScreen(Screen parent, Waypoint editing) {
		super(editing == null ? I18n.tr("waypoint.create") : I18n.tr("waypoint.edit"));
		this.parent = parent;
	...
	protected void draw(Gfx g, int mouseX, int mouseY, float partialTick) {
		...
		g.text(font, editing == null ? "§l" + I18n.tr("waypoint.create") : "§l" + I18n.tr("waypoint.edit"), x, y, Brand.TEXT, false);
	...
	private String playerPosition() {
		...
		return (int) Math.floor(Mc.x(minecraft.player)) + " / " + (int) Math.floor(Mc.y(minecraft.player))
				+ " / " + (int) Math.floor(Mc.z(minecraft.player));
	}
	...
	private void apply() {
		...
		if (editing == null) {
			...
			Mc.actionBar(I18n.tr("waypoint.created"));
		} else {
