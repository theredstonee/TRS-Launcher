 ...
 * Die eingehenden Nachrichten kommen aus {@code ChatMixin} – ohne Mixin (Forge 1.14.4) bleiben
 * Zeitstempel, Zusammenfassen, Kopieren und Auto-GG wirkungslos (nur die Text-Hotkeys gehen).
 */
public final class ChatFeatures {
				...
				if (count > 1) {
					...
					try {
						...
						if (ChatLines.removeNewestIfSame(lastAdded)) {
							result = Mc.concat(message, Mc.styled(ChatStacker.suffix(count), ChatFormatting.GRAY));
						} else {
							stacker.reset();
	...
	public void onHotkey(int index) {
		...
		if (text.isEmpty()) {
			Mc.actionBar(I18n.tr("chat.hotkeyEmpty", index + 1));
			return;
		}
		if (!hotkeys.tryAcquire(System.currentTimeMillis())) {
			Mc.actionBar(I18n.tr("chat.hotkeyTooFast"));
			return;
		}
		Mc.sendChat(text);
	}
	...
	public boolean onChatClick(double mouseX, double mouseY, boolean control) {
		...
		try {
			...
			Mc.actionBar(I18n.tr("chat.lineCopied"));
			return true;
		} catch (RuntimeException e) {
