...
public final class MinimapHud extends HudElement {
	...
	public void draw(Gfx g, FontRenderer font, boolean preview) {
		...
		if (modules.minimapPlayers.get()) drawPlayers(g, cells, px, player, centerX, centerZ, rotation);
		if (modules.minimapWaypoints.get()) drawWaypoints(g, cells, px, centerX, centerZ, rotation);
		drawSelf(g, cells, px, rotation, player.rotationYaw);

		if (modules.minimapCoords.get()) {
			String text = HudFormat.coords(player.posX, player.posY, player.posZ);
			g.text(font, font.trimStringToWidth(text, size), 1, size + 3, textColor(), false);
		}
	}
	 ...
	 */
	private void drawPlayers(Gfx g, int cells, int px, EntityPlayer self, double centerX, double centerZ, double rotation) {
