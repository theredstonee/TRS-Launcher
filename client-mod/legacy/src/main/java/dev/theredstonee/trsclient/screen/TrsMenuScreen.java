...
public final class TrsMenuScreen extends TrsScreen {
	private static final String TITLE = "§lTRS Client";
	...
	public TrsMenuScreen(GuiScreen parent) {
		this.parent = parent;
		this.selected = TrsClient.get().menuModules().get(0);
	}
	...
	protected void draw(Gfx g, int mouseX, int mouseY, float partialTick) {
		...
		int active = 0;
		for (Module m : TrsClient.get().menuModules()) if (m.isEnabled()) active++;
		String status = active + "/" + TrsClient.get().menuModules().size() + " aktiv";
		...
		if (px + 10 + font.getStringWidth(hint) < packsX - 6) {
	...
	private void drawCards(Gfx g, int mx, int my, int x, int y, int w, int h) {
		...
		cardsH = h;
		int cardW = (w - GAP) / 2;
		List<Module> all = TrsClient.get().menuModules();
		...
		for (int i = 0; i < all.size(); i++) {
			...
			g.text(font, font.trimStringToWidth(m.name(), cardW - 16), cx + 6, cy + 4, Brand.TEXT, false);
			...
			if (m.isHud()) g.text(font, "HUD", cx + cardW - 6 - font.getStringWidth("HUD"), pillY + 2, Brand.TEXT_DIM, false);
		}
		g.noScissor();
	...
	private void drawSettings(Gfx g, int mx, int my, int x, int y, int w, int h) {
		...
		g.text(font, "§l" + selected.name(), ix, cy, Brand.TEXT, false);
		int pillX = x + w - 7 - 26;
		...
		for (String line : font.listFormattedStringToWidth(selected.description(), iw)) {
		...
		if (selected instanceof HudModule) {
			...
			button(g, mx, my, ix, by, font.getStringWidth(label) + 10, 13, label, false, hud::resetPosition);
		} else if (selected == modules.crosshair) {
			String label = "Fadenkreuz bearbeiten";
			button(g, mx, my, ix, by, font.getStringWidth(label) + 10, 13, label, true, () -> open(new CrosshairEditorScreen(this)));
		}
	}
	...
	protected boolean onKey(int key, char typed) {
		if (key != 0 && key == TrsKeys.menu.getKeyCode()) {
			onClose();
			return true;
		}
		return false;
