...
public final class SettingRows {
	 ...
	 */
	public static int draw(Gfx g, FontRenderer font, List<Setting> settings, int x, int y, int w, int maxY,
		...
		for (Setting s : settings) {
			...
			if (s instanceof BoolSetting) {
				...
				g.text(font, font.trimStringToWidth(t.display(), tw - 6), tx + 3, y + 3,
	...
	private static void number(Gfx g, FontRenderer font, int right, int y, final NumberSetting n, double mx, double my, Hotspots hot) {
		int box = 11;
		int valueW = 30;
		int plusX = right - box;
	...
	private static void small(Gfx g, FontRenderer font, int x, int y, int size, String label, double mx, double my) {
