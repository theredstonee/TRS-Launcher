...
public final class CrosshairRenderer {
	...
	public boolean visibleInGame() {
		...
		return p != null && !p.isSpectator() && !Mc.hudHidden() && Mc.firstPerson();
	}
	...
	public void drawInGame(Gfx g) {
		...
		int cy = g.height() / 2;
		draw(g, cx, cy);
		// Angriffs-Abklingzeit gibt es erst ab 1.9 (davor liefert Mc immer \"aus\").
		if (modules.crosshairAttack.get() && Mc.attackIndicatorAtCrosshair()) {
			float scale = Mc.attackStrength(Mc.player());
			if (scale < 1.0F) {
