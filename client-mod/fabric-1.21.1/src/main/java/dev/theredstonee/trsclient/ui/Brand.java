...
public final class Brand {
	...
	public static int pill(GuiGraphics g, Font font, int x, int y, boolean on, boolean hover) {
		...
		int tx = on ? x + 3 : x + 6;
		g.drawString(font, label, tx, y + 2, on ? BG : TEXT, false);
		return w;
	}

	/** Umrandeter Text-Button. */
	public static void button(GuiGraphics g, Font font, int x, int y, int w, int h, String label, boolean primary, boolean hover) {
		if (primary) {
		...
		g.drawCenteredString(font, label, x + w / 2, y + (h - 8) / 2, primary ? 0xFFFFFFFF : TEXT);
	}
}
