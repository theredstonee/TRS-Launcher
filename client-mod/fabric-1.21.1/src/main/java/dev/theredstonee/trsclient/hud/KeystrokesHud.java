...
public final class KeystrokesHud extends HudElement {
	...
	public void draw(GuiGraphics g, Font font, boolean preview) {
		...
		key(g, font, KEY + GAP, 0, KEY, KEY, label(font, 0, move[0]), move[0].isDown(), bg, color);
		int y = KEY + GAP;
		for (int i = 1; i < 4; i++) {
			key(g, font, (i - 1) * (KEY + GAP), y, KEY, KEY, label(font, i, move[i]), move[i].isDown(), bg, color);
		}
		y += KEY + GAP;
		...
		mouse(g, font, 0, y, mh, "LMT", mc.options.keyAttack.isDown(), bg, color,
				showCps ? TrsClient.get().leftClicks().count(now) : -1);
		mouse(g, font, MOUSE_W + GAP, y, mh, "RMT", mc.options.keyUse.isDown(), bg, color,
	...
	private static void key(GuiGraphics g, Font font, int x, int y, int w, int h, String label, boolean down, boolean bg, int color) {
		fillKey(g, x, y, w, h, down, bg);
		int tw = font.width(label);
		g.drawString(font, label, x + (w - tw) / 2, y + (h - 8) / 2, down ? Brand.BG : color, !bg && !down);
	}

	private static void mouse(GuiGraphics g, Font font, int x, int y, int h, String label, boolean down, boolean bg, int color, int cps) {
		fillKey(g, x, y, MOUSE_W, h, down, bg);
		int textColor = down ? Brand.BG : color;
		boolean shadow = !bg && !down;
		if (cps < 0) {
			g.drawString(font, label, x + (MOUSE_W - font.width(label)) / 2, y + (h - 8) / 2, textColor, shadow);
		} else {
			String cpsText = CPS_TEXT[Math.min(cps, CPS_TEXT.length - 1)];
			g.drawString(font, label, x + (MOUSE_W - font.width(label)) / 2, y + 5, textColor, shadow);
			g.drawString(font, cpsText, x + (MOUSE_W - font.width(cpsText)) / 2, y + 16, textColor, shadow);
		}
	}

	private static void fillKey(GuiGraphics g, int x, int y, int w, int h, boolean down, boolean bg) {
