...
public final class InfoHuds {
	...
	public static final class Packs extends LinesHudElement {
		...
		@Override
		protected void build(boolean preview) {
			List<ResourcePackInfoClient> selected = new ArrayList<ResourcePackInfoClient>(mc.getResourcePackList().getEnabledPacks());
			Collections.reverse(selected);
			int count = 0;
			for (ResourcePackInfoClient pack : selected) {
				if (pack.isAlwaysEnabled() || pack.isOrderLocked()) continue;
				// func_195789_b() = Titel (in stable_47 noch ohne MCP-Namen)
