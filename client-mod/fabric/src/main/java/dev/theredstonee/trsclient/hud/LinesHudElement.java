...
public abstract class LinesHudElement extends HudElement {
	...
	private void refresh(Font font, boolean preview) {
		...
		build(preview);
		maxWidth = 0;
		for (String l : lines) maxWidth = Math.max(maxWidth, font.width(l));
	}
	...
	public void draw(Gfx g, Font font, boolean preview) {
		...
		if (bg) g.fill(0, 0, width(font, preview), height(font, preview), Brand.HUD_BG);
		int text = textColor();
		for (int i = 0, n = lines.size(); i < n; i++) {
			int c = colors.get(i);
			g.text(font, lines.get(i), PAD_X, PAD_Y + i * LINE_H, c == 0 ? text : c, !bg);
		}
	}
}
