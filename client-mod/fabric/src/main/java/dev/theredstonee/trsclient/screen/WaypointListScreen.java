...
public final class WaypointListScreen extends TrsScreen {
	private static final Component TITLE = Mc.text("Wegpunkte").withStyle(ChatFormatting.BOLD);
	...
	protected void draw(Gfx g, int mouseX, int mouseY, float partialTick) {
		...
		g.text(font, Gfx.clip(font, world.isEmpty() ? "keine Welt" : world, pw / 2), px + pw - 10 - Math.min(pw / 2,
		...
		if (list.isEmpty()) {
			g.text(font, "Noch keine Wegpunkte – Taste " + Mc.keyName(dev.theredstonee.trsclient.TrsKeys.waypointAdd)
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (minecraft != null && minecraft.player != null) {
				info += "   " + Projection.distanceLabel(waypoint.distanceTo(Mc.x(minecraft.player),
						Mc.y(minecraft.player), Mc.z(minecraft.player)));
			}
