...
public final class HudEditorScreen extends Screen {
	...
	public void renderBackground(GuiGraphics g) {
		// Kein dunkler Verlauf – das Spiel soll sichtbar bleiben.
		if (minecraft.level == null) renderDirtBackground(g);
	...
	public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
		...
		for (HudElement e : hud.elements()) {
			...
			if (e == hovered) {
				...
				g.drawString(font, name, b[0], ly, Brand.AMBER, true);
			}
		}
		...
		g.drawCenteredString(font, HEADLINE, width / 2, by + 4, Brand.TEXT);
		g.drawCenteredString(font, font.plainSubstrByWidth(HELP, boxW - 8), width / 2, by + 15, Brand.TEXT_DIM);
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		if (e == null) return super.mouseClicked(mouseX, mouseY, button);
		if (button == 1) {
	...
	public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
		if (dragging == null || button != 0) return super.mouseDragged(mouseX, mouseY, button, dragX, dragY);
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double scrollY) {
		HudElement e = elementAt(mouseX, mouseY);
		if (e == null || scrollY == 0) return super.mouseScrolled(mouseX, mouseY, scrollY);
