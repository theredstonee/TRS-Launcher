...
/** TRS-Client-Garderobe (Outfits, Favoriten) speichern (letzter Schreiber gewinnt, sonst 409 `stale` mit `current`). */
export default defineEventHandler(async (event) => {
  const auth = requireUser(event, 'sync')
  const body = await readJson(event, syncPresetsBody, SYNC_PRESETS_BODY_LIMIT)
  const ctx = useCtx()
  return { wardrobe: putDoc(ctx, auth.uuid, 'wardrobe', body.data, clientTime(ctx, body.updatedAt)) }
})
